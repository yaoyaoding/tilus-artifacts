...
def cuda_memset(addr: int, value: int, nbytes: int) -> None:
    ...
    return hidet.cuda.memset_async(addr, value, nbytes, hidet.ffi.runtime_api.get_current_cuda_stream())


# Use size_t type because the hip_malloc takes a size_t input and returns void* output.
@register_runtime_callback(restype=c_uint64, argtypes=[c_uint64])
