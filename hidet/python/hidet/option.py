...
def register_hidet_options():
    ...
    register_option(
        ...
        type_hint='bool',
        default_value=False,
        description='Control single-precision denormals support.',
    )
    register_option(
        name='cuda.dump_ptx',
        type_hint='bool',
        default_value=False,
        description='Whether to dump the PTX code to the cache directory when compiling CUDA kernels.',
    )
    register_option(
        name='cuda.dump_sass',
        type_hint='bool',
        default_value=False,
        description='Whether to dump the SASS code to the cache directory when compiling CUDA kernels.',
    )
    register_option(
        name='hip.arch',
        type_hint='str',
    ...
    register_option(
        name='internal.eager_expr_simplify',
        type_hint='bool',
        default_value=True,
        description='Whether to simplify the expressions eagerly.',
    )
    register_option(
...
class cuda:
    ...
    @staticmethod
    def ftz(enabled: bool):
        """
        Control single-precision denormals support.

        --ftz=true will flush denormals to zero and --ftz=false will preserve denormals values.

        """
        OptionContext.current().set_option('cuda.ftz', enabled)

    @staticmethod
    def dump_ptx(enabled: bool = True):
        """
        Dump the PTX code to the cache directory when compiling CUDA kernels.

        Parameters
        ----------
        enabled: bool
            Whether to dump the PTX code.
        """
        OptionContext.current().set_option('cuda.dump_ptx', enabled)

    @staticmethod
    def dump_sass(enabled: bool = True):
        """
        Dump the SASS code to the cache directory when compiling CUDA kernels.

        Parameters
        ----------
        enabled: bool
            Whether to dump the SASS code.
        """
        OptionContext.current().set_option('cuda.dump_sass', enabled)
...
class internal:
    @staticmethod
    def eager_expr_simplify(enable: bool = True):
        OptionContext.current().set_option('internal.eager_expr_simplify', enable)
