...
try:
    ...
    raise RuntimeError(
        ...
        "\n"
        "    $ python3 -m pip install -i https://test.pypi.org/simple hip-python>=5.7.1\n"
        "\n"
        "and try again"
    ) from e
...
if available():
    # a workaround to avoid hip use the libstdc++.so in conda env which has a low version.
    ctypes.cdll.LoadLibrary(ctypes.util.find_library('amdhip64'))
