...
from typing import TypeVar, Iterable, Tuple, List, Union, Sequence, Optional, Any
import cProfile
import contextlib
import io
import itertools
...
def iter_grid(shape: List[int]) -> Iterable[List[Any]]:
...
def error_tolerance(a: Union[np.ndarray, 'Tensor'], b: Union[np.ndarray, 'Tensor']) -> float:
    ...
    if isinstance(b, Tensor):
        b = b.torch()
    a = torch.asarray(a).cuda().float()
    b = torch.asarray(b).cuda().float()

    lf = 0.0
    rg = 10.0
    for _ in range(20):
        mid = (lf + rg) / 2.0
        if torch.allclose(a, b, rtol=mid, atol=mid):
            rg = mid
        else:
            lf = mid
    return (lf + rg) / 2.0
...
def get_callable_name(callable_obj) -> str:
