...
class CompiledFunction:
        ...
        if status is not None:
            ...
            for arg in args:
                if isinstance(arg, torch.Tensor):
                    arg_docs.append('torch.Tensor({}, {}, {})'.format(arg.dtype, arg.shape, arg.device))
                else:
                    arg_docs.append(str(arg))
            msg = 'Calling {}({}) failed. error:\n{}'.format(self.name, ', '.join(arg_docs), status)
    ...
    def profile(self, *args, warmup=1, repeat=10):
        """
        ...
        return benchmark_func(lambda: self.ctypes_func(*args), warmup=warmup, repeat=repeat, median=False)
...
class CompiledModule:
    ...
    def __call__(self, *args):
        ...
        ret: Optional[Union[int, float, bool]]
            The return value of the function.
        """
        launch_fn = self.functions.get('launch', None)
