...
class CompiledGraph:
    ...
    def __call__(self, *args, outputs=None):
        """
        ...
        outputs: Sequence[hidet.Tensor], optional
            The output tensors.

        Returns
        -------
        ret: Union[hidet.Tensor, List[hidet.Tensor]], optional
            The output tensor(s) (only when outputs=None)
        """
        if outputs is None:
            ...
            self.run_async(args, outputs=outputs)
    ...
    def _prepare_workspace(self):
        ...
        if hidet.cuda.available() and (
            self.cuda_workspace is None or self.cuda_workspace.num_bytes < required_cuda_workspace
        ):
    ...
    def _run_fast_path(self, inputs, outputs, symbol_dims: Tuple[int, ...]):
    ...
    def run_async(self, inputs, *, outputs=None):
        """
        ...
        if symbol_dims not in self.dispatch_table:
            self._run_slow_path(inputs, symbol_dims)

        return self._run_fast_path(inputs, outputs, symbol_dims)
