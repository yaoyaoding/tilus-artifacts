...
Stream = Union['hidet.cuda.Stream', 'hidet.hip.Stream']  # todo: make this as a parent class fo both stream classes
...
class Storage:
    ...
    def _convert(
        ...
        if src.device.is_cuda() and dst.device.is_cuda() and src.device.id != dst_device.id:
        ...
        elif src.device.is_cuda() or dst.device.is_cuda():
            # copy between a cuda gpu and host/same cuda gpu
        ...
        elif src.device.is_hip() or dst.device.is_hip():
            # Device is HIP. Note that for HIP, hidet.hip.memcpy manages both peer-to-peer copy and
            # host-to-device/device-to-host copy
            # For peer-to-peer copy, the source device context should be used.
        ...
        elif src.device.is_cpu() and dst.device.is_cpu():
            if non_blocking:
                if hidet.cuda.available():
                    ...
                    raise NotImplementedError('cpu memcpy support')
            else:
                if hidet.cuda.available():
                    hidet.cuda.memcpy(dst.addr, src.addr, src.num_bytes)
                elif hidet.hip.available():
            ...
            raise NotImplementedError(
                'Unsupported device type in Storage conversion: {} to {} (copy: {})'.format(
                    src.device, dst.device, copy
                )
            )
...
class DeviceMemoryPools:
    ...
    def __getitem__(self, device: Device) -> MemoryPool:
        if device not in self.device2pool:
            ...
            if not (hidet.cuda.available() or hidet.hip.available()):
                if device.is_cpu():
                    self.device2pool[device] = MemoryPool(
