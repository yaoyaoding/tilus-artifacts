...
class GenerateLaunchFuncPass(Pass):
    def process_module(self, ir_module: IRModule) -> IRModule:
        ...
        if len(kernel_functions) == 0:
            ...
            return ir_module
        if len(kernel_functions) > 1 and 'default_kernel' not in ir_module.attrs:
            raise ValueError(
                'Can only handle one kernel function in a module when `default_kernel` attr not specified.'
            )
        if 'default_kernel' in ir_module.attrs:
            kernel_name = ir_module.attrs['default_kernel']
            if kernel_name not in kernel_functions:
                raise ValueError('Can not find default kernel: {}'.format(kernel_name))
            kernel_func = kernel_functions[ir_module.attrs['default_kernel']]
        else:
