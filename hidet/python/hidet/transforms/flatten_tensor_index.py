...
class FlattenTensorAccessRewriter(IRRewriter):
    ...
    def visit_LetStmt(self, stmt: LetStmt):
        ...
        for bind_var, bind_value in zip(stmt.bind_vars, stmt.bind_values):
            if isinstance(bind_var.type, TensorType):
                size = simplify(bind_var.type.layout.size)
                var = Var(bind_var.hint, tensor_type(bind_var.type.dtype, [size], row_major(size)))
                self.memo[bind_var] = var
                bind_vars.append(var)
                bind_values.append(self(bind_value))
            elif isinstance(bind_var.type, TensorPointerType):
                size = simplify(bind_var.type.tensor_type.layout.size)
                var = Var(bind_var.hint, tensor_pointer_type(bind_var.type.tensor_type.dtype, [size], row_major(size)))
                self.memo[bind_var] = var
                bind_vars.append(var)
                bind_values.append(self(bind_value))
            else:
                ...
                bind_values.append(self(bind_value))
        return LetStmt(bind_vars, bind_values, body=self(stmt.body))
