...
from hidet.ir.type import TensorPointerType, TensorType, ArrayType, FuncType, type_equal
...
class NaiveLetStmtInlineRewriter(IRRewriter):
    ...
    def should_inline(self, var: Var, expr: Expr) -> bool:
        ...
        if not type_equal(var.type, self.type_infer(expr)):
            # type mismatch
            return False
        ...
        if isinstance(var.type, (TensorPointerType, TensorType)):
            ...
            # let v1 = v2
            if type_equal(var.type, expr.type):
                return True
            else:
                return False
        elif isinstance(expr, Constant):
