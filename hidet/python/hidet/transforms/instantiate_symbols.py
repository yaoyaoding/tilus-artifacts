...
from hidet.ir.primitives.runtime import get_symbol_value, get_symbol_value_ptr
            ...
            if func.kind == 'public':
                ...
                for symbol in ordered_symbols:
                    if symbol.type.is_data_type() and symbol.type == int32:
                        ...
                        symbol_values.append(cast(get_symbol_value_ptr(symbol.name), dtype=symbol.type))
                    else:
                        raise NotImplementedError(symbol.type)
                if len(symbol_params) > 0:
                    body = rewrite(body, {a: b for a, b in zip(symbol_params, symbol_values)})
            elif func.kind in [
                'cuda_kernel',
                'cuda_internal',
                'cpu_kernel',
