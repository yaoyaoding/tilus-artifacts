...
from hidet.ir.expr import Call, Expr, BinaryExpr, BitwiseAnd, BitwiseOr, BitwiseXor, cast
    ...
    def visit_Binary(self, e: BinaryExpr):
        ...
        if isinstance(lhs_dtype, DataType) and isinstance(rhs_dtype, DataType) and lhs_dtype.name != rhs_dtype.name:
            if isinstance(e, (BitwiseAnd, BitwiseOr, BitwiseXor)):
                dtype = max(lhs_dtype, rhs_dtype, key=lambda tp: (tp.nbits, not tp.signedness()))
            else:
