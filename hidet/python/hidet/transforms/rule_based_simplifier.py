...
class RuleBasedSimplifier(IRRewriter):
    ...
    def __init__(self, var2bound: Optional[Dict[Var, BoundInfo]] = None):
        super().__init__()
        self.analyzer = BoundAnalyzer(var2bound)
        ...
        c1, c2, c3 = any_constant(), any_constant(), any_constant()
        ec1, ec2 = any_expr(allow_const=True), any_expr(allow_const=True)
        self.args = {e1, e2, c1, c2, c3, ec1, ec2}
        true = constant(True, boolean)
        false = constant(False, boolean)
        self.patterns = [  # (pattern, target, [optional condition])
            # add
            ((c1 + e1) + e2, (e1 + e2) + c1),
            ((e1 + c1) + c2, e1 + (c1 + c2)),
            ((c1 - e1) + e2, (e2 - e1) + c1),
