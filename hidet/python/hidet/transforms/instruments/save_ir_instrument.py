...
class SaveIRInstrument(PassInstrument):
    def __init__(self, out_dir: str, save_for_transform=False):
        self.out_dir = out_dir
        self.index = 0
        self.transform_index = 0
        self.save_for_transform = save_for_transform
    ...
    def after_transform(self, pass_name: str, transform_name: str, obj: Union[IRModule, Function]):
        if self.save_for_transform:
            with open(
                os.path.join(
                    self.out_dir, '{}_{}_{}_{}.txt'.format(self.index, pass_name, self.transform_index, transform_name)
                ),
                'w',
            ) as f:
                f.write(str(obj))
