...
class SourceCompiler:
    ...
    def run_compile_command(self, command: str, src_path, out_dir: str, keep_files: Sequence[str] = ()):
        """
        Run the `command` to compile the source code in 'src_path' file.

        This function runs the command in a temporary directory, and generates the following files to `out_dir` directory:
        - `compile.sh` the shell script that runs the compilation command.
        - `compile_output.txt` the stdout and stderr of the compilation command.
        - All files specified in `keep_files` list are copied from the working directory to `out_dir` directory.

        A CompilationFailed exception is raised if the command fails with non-zero return code.
        ...
        out_dir: str
            The output directory to store above files.

        keep_files: Sequence[str]
            The list of files to keep after the compilation.
        """
        try:
            # write the compilation command to "compile.sh"
            with open(os.path.join(out_dir, 'compile.sh'), 'w') as f:
                f.write("#!/bin/bash\n\n")
                f.write(command)
                f.write("\n")

            # run the compilation command in a temporary directory
            with tempfile.TemporaryDirectory() as working_dir:
                ...
                for keep_file in keep_files:
                    # keep the file with pattern `keep_file` with glob
                    matched_files: List[str] = glob.glob(keep_file, root_dir=working_dir, recursive=True)
                    if matched_files:
                        for matched_file in matched_files:
                            shutil.copy(
                                os.path.join(working_dir, matched_file),
                                os.path.join(out_dir, os.path.basename(matched_file)),
                            )
                    else:
                        warnings.warn(
                            f'File {keep_file} is not found in the working directory. Did the compilation command: \n'
                            + command
                            + '\n'
                            + 'generate the file?'
                        )
...
class NVCC(SourceCompiler):
    ...
    def compile(
        ...
        ftz = hidet.option.get_option('cuda.ftz')
        ...
        command = [
            ...
            *['-l{}'.format(library) for library in linking_libs],
            ...
            '-O3',
            # host compiler options: enable position-independent-code, unroll loops, fast math
            '--compiler-options -fPIC,-m64,-march={cpu_arch},-O3,-funroll-loops,-ffast-math'.format(cpu_arch=cpu_arch),
            # use c++11 standard
            '-std=c++11',
            # keep intermediate files and interleave source in PTX when dumping PTX code.
            '--keep --source-in-ptx' if hidet.option.get_option('cuda.dump_ptx') else '',
            ...
            '-ftz={}'.format('true' if ftz else 'false'),
            '-prec-div=false',
            # we assert that all pointers in cuda program are __restrict__ to allow nvcc perform more optimizations.
            '--restrict',
        ...
        out_dir = os.path.dirname(out_lib_path)
        keep_files = []
        if hidet.option.get_option('cuda.dump_ptx'):
            source_name = os.path.basename(src_path).split('.')[0]  # we assume the source file has format is "a.b"
            keep_files.append(f'{source_name}.ptx')
        self.run_compile_command(" ".join(command), src_path, out_dir, keep_files=keep_files)

        # print warnings if there are any
        nvcc_output_log = os.path.join(out_dir, 'compile_output.txt')
        with open(nvcc_output_log, 'r') as f:
            lines = f.readlines()
        ...
        # dump sass code
        if hidet.option.get_option('cuda.dump_sass') and os.path.exists('/usr/local/cuda/bin/cuobjdump'):
            source_name = os.path.basename(src_path).split('.')[0]
            dump_command = f'/usr/local/cuda/bin/cuobjdump -sass {out_lib_path}'
            with open(os.path.join(out_dir, f'{source_name}.sass'), 'w') as f:
                subprocess.run(dump_command.split(), stdout=f, check=False)
...
class HIPCC(SourceCompiler):
    ...
    def compile(
        ...
        command = [
            ...
            self.hipcc_path,
            # dump the intermediate representation of the source code.
            '-save-temps',
            # the included directories.
            *['-I{}'.format(include_dir) for include_dir in self.include_dirs + list(include_dirs)],
            # the library directories.
        ...
        out_dir = os.path.dirname(out_lib_path)
        self.run_compile_command(" ".join(command), src_path, out_dir, keep_files=['*amdgcn*.s'])
...
class GCC(SourceCompiler):
    ...
    def compile(
        ...
        out_dir = os.path.dirname(out_lib_path)
        self.run_compile_command(" ".join(command), src_path, out_dir)
