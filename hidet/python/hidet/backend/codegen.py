...
from hidet.utils import prod, remove_parathesis
...
class Codegen(ModuleFunctor, StmtFunctor, ExprFunctor, TypeFunctor):
    ...
    def scalar_literal(self, value, dtype: DataType):
        if dtype == dtypes.boolean:
            ...
            ret = 'float8_e4m3({})'.format(float(value))
    ...
    def param_declare(self, v: Var):
        ...
        if isinstance(self, HIPCodegen):
            pointer_star = ' * __restrict__ '
        ...
        if isinstance(v_type, DataType):
            ...
            if v_type.use_bracket:
                ...
                return attr_doc + base_type_doc + pointer_star + name_doc
        elif isinstance(v_type, TensorPointerType):
            dtype = v_type.tensor_type.dtype
            base_type_doc = self(dtype)
            return base_type_doc + pointer_star + name_doc
    ...
    def visit_List(self, lst: List):
        return doc_join([remove_parathesis(self(v)) for v in lst], ', ')

    def visit_Tuple(self, tp: Tuple):
        return doc_join([remove_parathesis(self(v)) for v in tp], ', ')
    ...
    def visit_TensorElement(self, e: TensorElement):
        ...
        index_doc = doc_join(['[' + remove_parathesis(self(idx)) + ']' for idx in e.indices], '')
    ...
    def visit_Cast(self, e: Cast):
        ...
        if isinstance(src_dtype, DataType) and isinstance(dst_dtype, DataType) and src_dtype == dtypes.float16:
            ...
            proxy = {
                ...
                dtypes.int8: 'short',
                dtypes.uint8: 'unsigned short',
            }
            if dst_dtype in proxy:
                return (
                    '(('
                    + self.visit(dst_dtype)
                    + ')('
                    + proxy[dst_dtype]
                    + ')('
                    + remove_parathesis(self(e.expr))
                    + '))'
                )
            else:
                return '((' + self.visit(dst_dtype) + ')(' + remove_parathesis(self(e.expr)) + '))'
        if isinstance(src_dtype, DataType) and isinstance(dst_dtype, DataType) and src_dtype == dtypes.float8_e4m3:
            if dst_dtype == dtypes.float32:
                return '((float)(' + self(e.expr) + '))'
            elif dst_dtype == dtypes.float16:
                return '((' + self(e.expr) + ').operator half())'
            elif dst_dtype == dtypes.bfloat16:
                return '((' + self(e.expr) + ').operator bfloat16_t())'
            else:
                return Text('((') + self.visit(e.target_type) + ')(' + remove_parathesis(self(e.expr)) + '))'
        else:
            return Text('((') + self.visit(e.target_type) + ')(' + remove_parathesis(self(e.expr)) + '))'
    ...
    def visit_Call(self, e: Call):
        ...
        if func_name in self.ir_module.functions:
            ...
            param_doc = Text('(') + doc_join([remove_parathesis(self(arg)) for arg in e.args], Text(', ')) + ')'
            ...
            return entry.codegen_name + (
                Text('(') + doc_join([remove_parathesis(self(arg)) for arg in e.args], Text(', ')) + ')'
            )
        else:
            ...
            return (
                self(e.func_var)
                + Text('(')
                + doc_join([remove_parathesis(self(arg)) for arg in e.args], Text(', '))
                + Text(')')
            )
    ...
    def visit_BufferStoreStmt(self, stmt: BufferStoreStmt):
        ...
        for idx in stmt.indices:
            doc += '[' + remove_parathesis(self(idx)) + ']'
        doc += Text(' = ') + remove_parathesis(self(stmt.value)) + ';'
        return doc

    def visit_AssignStmt(self, stmt: AssignStmt):
        return NewLine() + self(stmt.var) + ' = ' + remove_parathesis(self(stmt.value)) + ';'

    def visit_LetStmt(self, stmt: LetStmt):
        doc = Doc()
        for bind_var, bind_value in zip(stmt.bind_vars, stmt.bind_values):
            doc += NewLine() + self.local_var_declare(bind_var) + ' = ' + remove_parathesis(self(bind_value)) + ';'
        doc += self(stmt.body)
        return doc
    ...
    def visit_ForStmt(self, stmt: ForStmt):
        ...
        cond_doc = remove_parathesis(self(v < stmt.extent))
        update_doc = self(v) + ' = ' + remove_parathesis(self(v + 1))
        doc = Text('')
        if stmt.attr.unroll:
    ...
    def visit_WhileStmt(self, stmt: WhileStmt):
        doc = NewLine() + Text('while (') + remove_parathesis(self(stmt.cond)) + ')'
        body_doc = self(stmt.body)
        doc += Text(' {') + body_doc.indent() + NewLine() + Text('} ')
        return doc
    ...
    def visit_IfStmt(self, stmt: IfStmt):
        doc = NewLine() + Text('if (') + remove_parathesis(self(stmt.cond)) + ') '
    ...
    def visit_AsmStmt(self, stmt: AsmStmt):
        ...
        # if '\n' in stmt.template_string:
        lines = stmt.template_string.split('\n')
        lines = ['"' + line.strip() + '"' for line in lines if line.strip() != '']
        if len(lines) > 1:
            lines = [NewLine()] + lines
        template_doc = doc_join(lines, NewLine())
...
class CUDACodegen(Codegen):
    ...
    def visit_Function(self, func: Function) -> Doc:
        ...
        if func.kind == 'public' and self.ir_module.namespace == '' and not body_doc.empty():
            ...
            if func.ret_type.is_pointer():
                ret = '0'
            elif func.ret_type.is_void():
                ret = ''
            elif func.ret_type.is_data_type():
                dtype = func.ret_type.as_data_type()
                if dtype.is_integer():
                    ret = '0'
                elif dtype.is_float():
                    ret = '0.0'
...
class HIPCodegen(Codegen):
    def require_headers(self) -> Doc:
        ...
        if self.require_fp8:
            doc += Text('#include <hidet/runtime/hip/float8.h>') + NewLine()
        if self.require_bf16:
            doc += Text('#include <hip/hip_bf16.h>') + NewLine()
        doc += Text('#include <hidet/runtime/hip/context.h>') + NewLine()
        doc += Text("#include <hidet/runtime/logging.h>") + NewLine()

        if self.require_bf16:
            doc += Text('typedef __hip_bfloat16 bfloat16_t;') + NewLine()
    ...
    def visit_Cast(self, e: Cast):
        ...
        if not isinstance(src_type, DataType) or not isinstance(dst_type, DataType):
            return super().visit_Cast(e)

        # conversion between float and bfloat16, function defined in <hip/hip_bf16.h>
        if src_type == dtypes.bfloat16 and dst_type == dtypes.float32:
            return Text('__bfloat162float({})'.format(self(e.expr)))
        elif src_type == dtypes.float32 and dst_type == dtypes.bfloat16:
            return Text('__float2bfloat16({})'.format(self(e.expr)))
        elif src_type != dtypes.float32 and dst_type in [dtypes.float8_e4m3, dtypes.float8_e5m2]:
            # use float32 as a bridge data type
            return self.visit(Cast(Cast(e.expr, dtypes.float32), e.target_type))
    ...
    def visit_Constant(self, e: Constant):
        ...
        if dtype == dtypes.bfloat16:
            return Text('__float2bfloat16({})'.format(e.value))
        return super().visit_Constant(e)
