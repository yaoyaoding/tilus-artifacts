...
def dtype_from_torch(torch_dtype: Union['torch.Tensor', 'torch.dtype', None]) -> Optional[DataType]:
    if torch_dtype is None:
        return None
    return _dtype_from_torch(getattr(torch_dtype, 'dtype', torch_dtype))


@functools.cache
def _dtype_from_torch(torch_dtype) -> Optional[DataType]:
    if not available():
...
def symbol_like_torch(tensor) -> Tensor:
    ...
    if isinstance(tensor, FakeTensor):
        ...
        return hidet.symbol(
            ...
            dtype=dtype_from_torch(tensor.dtype).name,
            device=device_from_torch(tensor.device.type),
        )
