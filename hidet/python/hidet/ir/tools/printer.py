...
from hidet.ir.tile.ops import UnaryTileOp, BinaryTileOp, ReduceOp, Dot, DebugPrint, Assign, CastOp
from hidet.ir.tile.ops import AllocTensor, InsertSliceAsync, AsyncWait, AsyncCommitGroup, ExtractSlice, ProcedureOp
...
class IRPrinter(IRFunctor):
    def __init__(self, inline_attrs: bool = False):
        ...
        self.inline_attrs: bool = inline_attrs
    ...
    def _expand_compose_layout(self, seq: List[TileLayout]) -> List[TileLayout]:
        # import hidet.ir.tile.layout.ComposedLayout, which is different the hidet.ir.layout.ComposedLayout
        from hidet.ir.tile.layout import ComposedLayout  # pylint: disable=redefined-outer-name, reimported

        ret = []
        for item in seq:
            if isinstance(item, ComposedLayout):
                ret.extend(self._expand_compose_layout([item.outer, item.inner]))
            else:
                ret.append(item)
        return ret
    ...
    def visit_TileLayout(self, layout: TileLayout):
        ...
        # the following tile layout may share the name with the ones in hidet.ir.layout
        from hidet.ir.tile.layout import BlockLayout, FlattenBlockLayout, BlockDotOperandLayout
        from hidet.ir.tile.layout import MmaOutputLayout, MmaDotOperandLayout, SwizzleLayout, ComposedLayout
        from hidet.ir.tile.layout import AtomLayout, RepeatLayout, SpatialLayout, ExpandLayout, ReduceLayout
        from hidet.ir.tile.layout import MmaDotOperandSharedLayout, SliceLayout
        ...
        if isinstance(layout, BlockLayout):
            ...
            attr_string = str(doc)
            return self.get_attr_abbr(hint='block', attr_string=attr_string)
        elif isinstance(layout, FlattenBlockLayout):
            doc = 'flatten_block(' + self.visit_TileLayout(layout.parent) + ', axis=' + self(layout.axis) + ')'
            return self.get_attr_abbr(hint='flatten_block', attr_string=str(doc))
        elif isinstance(layout, RepeatLayout):
            if layout.flatten_local and same_list(layout.ranks, range(len(layout.shape))):
                return 'repeat(' + self(layout.shape) + ')'
            else:
                return (
                    'repeat('
                    + self(layout.shape)
                    ...
                    + self(layout.ranks)
                    + '], flatten='
                    + self(layout.flatten_local)
                    + ')'
                )
        elif isinstance(layout, SpatialLayout):
            if same_list(layout.ranks, range(len(layout.shape))) and layout.flatten_local:
                return 'spatial(' + self(layout.shape) + ')'
            else:
                return (
                    'spatial('
                    + self(layout.shape)
            ...
            sub_items = {
                'shape': self(layout.shape),
                'worker_shape': self(layout.worker_shape),
                'ranks': self(layout.ranks),
                'worker_ranks': self(layout.worker_ranks),
                'flatten_local': self(layout.flatten_local),
                'squeeze_dims': self(layout.squeeze_dims),
            }
            doc = 'atom(' + doc_join([k + '=[' + v + ']' for k, v in sub_items.items()], ', ') + ')'
            attr_string = str(doc)
            return self.get_attr_abbr(hint='atom', attr_string=attr_string)
        elif isinstance(layout, ExpandLayout):
            doc = 'expand(' + self.visit_TileLayout(layout.base) + ', dim=' + self(layout.dim) + ')'
            return doc
        elif isinstance(layout, SwizzleLayout):
            doc = (
                'swizzle('
                + self.visit_TileLayout(layout.base)
                + ', dim='
                + self(layout.dim)
                + ', regards_dim='
                + self(layout.regards_dim)
                + ', log_step='
                + self(layout.log_step)
                + ')'
            )
            return doc
        elif isinstance(layout, ComposedLayout):
            layouts = self._expand_compose_layout([layout.outer, layout.inner])
            doc = doc_join([self.visit_TileLayout(l) for l in layouts], ' * ')
            return self.get_attr_abbr(hint='layout', attr_string=str(doc))
        elif isinstance(layout, BlockDotOperandLayout):
            doc = (
                'block_dot_operand(parent=' + self.visit_TileLayout(layout.parent) + ', id=' + self(layout.op_idx) + ')'
            )
            return self.get_attr_abbr(hint='simt_operand', attr_string=str(doc))
        elif isinstance(layout, MmaDotOperandLayout):
            doc = 'mma_dot_operand(parent=' + self.visit_TileLayout(layout.mma) + ', id=' + self(layout.op_idx) + ')'
            return self.get_attr_abbr(hint='mma_operand', attr_string=str(doc))
        elif isinstance(layout, MmaOutputLayout):
            items = {
                'shape': '{}x{}x{}'.format(layout.m_size, layout.n_size, layout.k_size),
                'warp_shape': '{}x{}x{}'.format(
                    layout.repeat_m * layout.inst_m, layout.repeat_n * layout.inst_n, layout.repeat_k * layout.inst_k
                ),
                'inst_shape': '{}x{}x{}'.format(layout.inst_m, layout.inst_n, layout.inst_k),
                'in_dtype': str(layout.config.in_dtype.short_name),
                'out_dtype': str(layout.config.out_dtype.short_name),
            }
            doc = 'mma(' + doc_join([k + '=' + v for k, v in items.items()], ', ') + ')'
            attr_string = str(doc)
            return self.get_attr_abbr(hint='mma', attr_string=attr_string)
        elif isinstance(layout, MmaDotOperandSharedLayout):
            return 'mma_operand_shared(' + self.visit_TileLayout(layout.mma_operand) + ')'
        elif isinstance(layout, ReduceLayout):
            layout_str = (
                'reduce('
                + self.visit_TileLayout(layout.parent)
                + ', axis='
                + self(layout.dim)
                + ', keep_dim='
                + str(layout.keep_dim)
            ...
            return self.get_attr_abbr(hint='reduce', attr_string=str(layout_str))
            ...
            return self.get_attr_abbr(hint='slice', attr_string=str(layout_str))
        else:
            raise NotImplementedError()
    ...
    def visit_TileType(self, t: TileType):
        ...
        if t.scope is not None and t.scope.is_shared():
            shape_items.append('smem')
...
def astext(obj: Node, inline_attrs=False) -> str:
    if isinstance(obj, Node):
        printer = IRPrinter(inline_attrs=inline_attrs)
