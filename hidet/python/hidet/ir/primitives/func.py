...
class PrimitiveFunctionPool:
    ...
    def lookup_by_name(self, name: str) -> PrimitiveFunctionRegistry:
        if name not in self.name2func:
            ...
            raise ValueError(
                'Can not find primitive function with key: {}, do you want:\n{}.'.format(
...
def register_primitive_function(
    name: Union[str, Function, Any], func_or_type=None, codegen_name: Optional[str] = None, generic=False
    ...
    if isinstance(name, Function):
        assert func_or_type is None and codegen_name is None
        func_or_type = name
        name = func_or_type.name
