...
@initialize()
def register_atomic_cas_functions():
    from hidet.lang import script, attrs, asm
    from hidet.lang.types import void_p

    for dtype in [u16, i32, u32, u64]:

        template = 'atom.cas.b{} %0, [%1], %2, %3;'.format(dtype.nbits)

        @script
        def cuda_atomic_cas(ptr: void_p, old: dtype, updated: dtype) -> dtype:
            attrs.func_kind = 'cuda_internal'
            attrs.func_name = 'cuda_atomic_cas_{}'.format(dtype.short_name)
            ret = dtype.zero
            asm(template, outputs=[ret], inputs=[ptr, old, updated], is_volatile=True)
            return ret

        register_primitive_function(name='cuda_atomic_cas_{}'.format(dtype.short_name), func_or_type=cuda_atomic_cas)
...
def atomic_cas(addr: Expr, compare: Union[Expr, int], value: Union[Expr, int]):
    ...
    if isinstance(addr_type, PointerType):
        dtype = addr_type.base_type
    else:
        raise NotImplementedError()
    return call_primitive_func('cuda_atomic_cas_{}'.format(dtype.as_data_type().short_name), [addr, compare, value])
