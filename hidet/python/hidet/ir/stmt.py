...
class BufferStoreStmt(Stmt):
    def __init__(self, buf, indices, value, protected=False):
        super().__init__()
        self.buf: Union[Expr, TensorNode] = buf
        self.indices = convert(indices)
        self.value = convert(value)
        self.protected = protected

        assert isinstance(indices, (list, tuple)) and all(isinstance(v, (int, Expr)) for v in indices), indices
        assert isinstance(buf, Expr), buf
...
class WhileStmt(Stmt):
    def __init__(self, cond: Expr, body: Optional[Stmt] = None):
        self.cond: Expr = cond
        self.body: Optional[Stmt] = body
...
class SeqStmt(Stmt):
    def __init__(self, seq: Sequence[Stmt]):
        super().__init__()
...
def launch_kernel(
    ...
    return LaunchKernelStmt(func_var, args, grid_dim, block_dim, convert(shared_mem), target)
