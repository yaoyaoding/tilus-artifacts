...
from hidet.ir.stmt import Stmt, EvaluateStmt, DeclareStmt, BufferStoreStmt, AssignStmt, LetStmt, ForStmt, ForMappingStmt
from hidet.ir.stmt import WhileStmt, BreakStmt, ContinueStmt, IfStmt, ReturnStmt, AsmStmt, AssertStmt, BlackBoxStmt
from hidet.ir.stmt import LaunchKernelStmt, SeqStmt
...
class StmtRewriter(StmtFunctor, BaseRewriter):
    def __init__(self, use_memo: bool = True):
        ...
        self.prologues: List[Stmt] = []

    def append_prologue_stmt(self, stmt: Stmt):
        self.prologues.append(stmt)
    ...
    def visit_dispatch(self, node: Node):
        ...
        if self.prologues:
            if isinstance(ret, SeqStmt):
                seq = tuple(self.prologues) + ret.seq
                ret = SeqStmt(seq)
            else:
                ret = SeqStmt(self.prologues + [ret])
            self.prologues = []
        return ret
    ...
    def visit_LaunchKernelStmt(self, stmt: LaunchKernelStmt):
        ...
        if same_list(
            ...
            return LaunchKernelStmt(func_var, args, grid_dim, block_dim, shared_mem_bytes, stmt.target)
