...
class Expr(Node):
    ...
    def _unary(cls, a):  # pylint: disable=bad-staticmethod-argument
        if not isinstance(a, Expr):
            a = convert(a)
        if not hidet.option.get_option('internal.eager_expr_simplify'):
            # disable eager expression simplification
            return cls(a)
    ...
    def _binary(cls, a: Expr, b: Expr):  # pylint: disable=bad-staticmethod-argument
        ...
        if isinstance(a, Constant) and isinstance(b, Constant):
            ...
            if a.type.is_data_type() and b.type.is_data_type():
                ...
                elif cls in [BitwiseAnd, BitwiseOr, BitwiseXor]:
                    return constant(value, a.type if a.type.nbits > b.type.nbits else b.type)
                else:
...
class SymbolVar(Var):
    ...
    def __init__(self, name: str, dtype: Union[DataType, PointerType]):
        from hidet.ir.dtypes import int32

        super().__init__(hint=None, type=dtype, name=name)

        assert dtype == int32 or isinstance(
            dtype, PointerType
        ), 'Currently, we only supports symbols with int32 or pointer type.'
...
def reinterpret(value: Expr, target_type: BaseType):
...
def symbol_var(name: str, dtype: Union[str, DataType, PointerType] = 'int32') -> SymbolVar:
    if isinstance(dtype, (str, DataType)):
        dtype = data_type(dtype)
