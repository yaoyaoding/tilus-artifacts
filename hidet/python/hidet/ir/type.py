...
class DataType(BaseType):
    ...
    def is_signed_integer(self) -> bool:
        return self.is_integer() and self.signedness()

    def is_unsigned_integer(self) -> bool:
        return self.is_integer() and not self.signedness()
...
def sizeof(tp: BaseType) -> int:
    ...
    elif isinstance(tp, TileType):
        return sizeof(tp.type) * prod(tp.shape)
    else:
        raise NotImplementedError(type(tp))


def base_type_of_pointer(ptr_type: Union[PointerType, TensorPointerType, TensorType]):
    if isinstance(ptr_type, PointerType):
        return ptr_type.base_type
    elif isinstance(ptr_type, TensorPointerType):
        return ptr_type.tensor_type
    elif isinstance(ptr_type, TensorType):
        return ptr_type.dtype
    else:
        raise ValueError('Unknown pointer type {}'.format(ptr_type))
