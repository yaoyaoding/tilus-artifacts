...
@pytest.mark.parametrize('device,tol', [['cpu', 1e-2], ['cuda', 1e-4]])
def test_attention(device, tol):
    ...
    for seq in [1, 2, 3, 4, 8]:
        ...
        for y in [y_dynamic, y_dynamic_opt]:
            numpy.testing.assert_allclose(y_static.cpu().numpy(), y.cpu().numpy(), atol=tol, rtol=tol)
